...
result <- jsonlite::fromJSON(file.path("./SCEPrep/CTIM-T","exp_data.json"))
total_counts <- result$totalCounts
data_ <- rhdf5::h5read(file.path("./SCEPrep/CTIM-T","data.h5"),"X")
dims_ <- rhdf5::h5readAttributes(file = file.path("./SCEPrep/CTIM-T","data.h5"), 
                                 name = "X")$shape
...
plotData <- (jsonlite::fromJSON(file.path("./SCEPrep/CTIM-T","plot_data.json"))$data) %>% 
  as.data.frame
...
gobs <- lapply(markers, function(i){
  DoHeatmap(avg.exp,features = i,group.by = "Cluster",size=3.5,draw.lines = F,angle = 90)+NoLegend()+ scale_fill_gradientn(colors = c("blue", "white", "red"))
})
g <- grid.arrange(grobs = gobs, ncol = 2)
ggsave(filename = "./SCEPrep/T_AVGEXP.pdf",plot = g,height = 60,width = 40,units = "cm")


g <- FeaturePlot(seurat_obj,features = c("CD3D","GZMK"),reduction = "umap")
ggsave("./SCEPrep/Check_Object.pdf",plot = g,height = 30,width = 40,units = "cm")
