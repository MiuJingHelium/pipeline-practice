...
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q martyomov -G compute-martyomov \
        -n 2 -o Object_Creation_NKILC_TS.out \
        -e Object_Creation_NKILC_TS.err -R 'rusage[mem=32GB] span[hosts=1]' \
	-J Object_Creation_NKILC_TS -M 32GB \
        -a "docker(kalisaz/scrna-extra:r4.3.0)" /bin/bash -c \
	"Rscript ./Object_Creation_NKILC_TissueSubset.R"
