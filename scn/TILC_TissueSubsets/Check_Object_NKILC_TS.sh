...
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q martyomov -G compute-martyomov \
        -n 2 -o Check_Object_NKILC_TS.out \
        -e Check_Object_NKILC_TS.err -R 'rusage[mem=32GB] span[hosts=1]' \
	-J Check_Object_NKILC_TS -M 32GB \
        -a "docker(kalisaz/scrna-extra:r4.3.0)" /bin/bash -c \
	"Rscript ./Check_Object_NKILC_TS.R"
