...
result <- jsonlite::fromJSON(file.path("./CTIM_CD4T_TS","exp_data.json"))
total_counts <- result$totalCounts
data_ <- rhdf5::h5read(file.path("./CTIM_CD4T_TS","data.h5"),"X")
...
plotData <- (jsonlite::fromJSON(file.path("./CTIM_CD4T_TS","plot_data.json"))$data) %>% 
  as.data.frame
...
ggsave(filename = "./AVGEXP_CD4_TS.pdf",plot = g,height = 60,width = 40,units = "cm")


g <- FeaturePlot(seurat_obj,features = c("CD4","FOXP3"),reduction = "umap")
ggsave("./Check_Object_CD4_TS.pdf",plot = g,height = 30,width = 40,units = "cm")
