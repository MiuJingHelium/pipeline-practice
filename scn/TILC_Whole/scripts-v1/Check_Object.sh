...
LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q martyomov -G compute-martyomov \
        -n 2 -o Check_Object.out \
        -e Check_Object.err -R 'rusage[mem=32GB] span[hosts=1]' \
	-J Check_Object -M 32GB \
        -a "docker(kalisaz/scrna-extra:r4.3.0)" /bin/bash -c \
	"Rscript /scratch1/fs1/martyomov/carisa/Cross_Tissue_Cell_Atlas/Check_Object.R"
