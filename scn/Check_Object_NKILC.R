...
result <- jsonlite::fromJSON(file.path("./CTIM_NKILC","exp_data.json"))
total_counts <- result$totalCounts
data_ <- rhdf5::h5read(file.path("./CTIM_NKILC","data.h5"),"X")
dims_ <- rhdf5::h5readAttributes(file = file.path("./CTIM_NKILC","data.h5"), 
                                 name = "X")$shape
...
plotData <- (jsonlite::fromJSON(file.path("./CTIM_NKILC","plot_data.json"))$data) %>% 
  as.data.frame
...
ggsave(filename = "./AVGEXP_NKILC.pdf",plot = g,height = 60,width = 40,units = "cm")


g <- FeaturePlot(seurat_obj,features = c("NCR1","NCAM1"),reduction = "umap")
ggsave("./Check_Object_NKILC.pdf",plot = g,height = 30,width = 40,units = "cm")
